...
<body>
	...
	<div class="container">
						...
						<div class="row mt-2">
							<div class="col-lg-6">
								...
								<div class="input-group ">
									<input name="qa_visit_date" value="{{\Carbon\Carbon::today()->format('Y-m-d')}}" type="text" class="form-control required-field border-clr">
							...
							<div class="col-lg-6">
								<label>County</label>
								<select name="county" id="county" class="required-field ">
									<option value="">Pick from Drop Down</option>
							...
							<div class="col-lg-6">
								...
								<select name="locality" id="locality" class="required-field locality">
									<option value="">Pick from Drop Down</option>
								</select>
							</div>
							<div class="col-lg-6">
								<label>Province</label>
								<select name="province" id="province" class="required-field ">
									<option value="">Pick from Drop Down</option>
								</select>
							</div>
                            <input type="hidden" name="client_id" value="{{$client->id}}">
							<div class="col-lg-6">
								<label>Enter Clients Number</label>
								<input type="text" name="client_number" readonly value="{{$client->id_number}}" class="required-field border-clr">
							</div>
							<div class="col-lg-6">
								<label>Client Health Diagnosis</label>
							...
							<div class="col-lg-6">
								...
								<input type="text" name="client_name" readonly="" value="{{$client->name}}" class="">
							</div>
							<div class="col-lg-6">
								<label>Clients Date of Birth</label>
								<div class="input-group ">
									<input name="client_dob" readonly value="{{$client->dob}}" type="text" class="form-control ">
									<div class="input-group-addon"> <span class="glyphicon glyphicon-th"></span></div>
								</div>
							</div>
							<div class="col-lg-6">
								...
								<div class="input-group">
									<input name="care_start_date" readonly value="{{$client->care_start_date}}" type="text" class="form-control">
									<div class="input-group-addon"> <span class="glyphicon glyphicon-th"></span></div>
								</div>
							</div>
							<div class="col-lg-6">
								@php
								$diff=\Carbon\Carbon::parse($client->care_start_date)->diff(\Carbon\Carbon::today()->format('Y-m-d'))->format('%m');
								@endphp

								<label>Duration of Visit (Months)</label>
								<div class="input-group ">
					...
					<div id="card-2" class="bg-white collapse" aria-labelledby="property-sold" data-parent="#formValues">
						<div class="row mt-2 outer-wrap" id="cart-item-1">
							...
							<div class="col-lg-6 care_giver_id">
								...
								<button type="button" class="btn search-care-giver" style="position: absolute;right: 20px;margin-top: 3px;"><i class="fa fa-search"></i> </button>
							</div>
							<div class="col-lg-6 care_giver_name">
								<label>Care Giver Name </label>
								<input type="text" name="care_giver_name[]" class="fl-3 required-field " />
							</div>
							<div class="col-lg-6">
				...
				<div class="card shadow-box incomplete">
					...
					<div id="card-6" class="collapse bg-white  " aria-labelledby="six" data-parent="#formValues">
						<div class="row mt-2">
							...
							<div class="col-lg-6">
								<label>Other Services Assistance Required? Yes/No </label>
							...
							<div class="col-lg-6 show-on-yes hidden">
								<label>Other Services Assistance </label>
								<select name="add_other" class="add_other required-field">
								</select>
							</div>
							<div class="col-lg-12">
						...
						<div class="row mt-2">
							...
							<div class="d-flex flex-column flex-sm-row align-items-center justify-content-sm-between col-lg-12">
								<button type="button" class="outline-btn back" data-id="10">
									<img src="assets/img/left-arrow.svg" alt="" />Back</button>
								<button id="submit" type="sub
																	mit" class="primary-btn">Submit</button>
							</div>
						</div>
					</div>
				</div>
		...
		<footer>
			<p>All Copyright Rights Reserved by Quality Assurance. </p>
		</footer>
	</div>
	<!-- js -->
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$.get(
				"https://script.google.com/macros/s/AKfycbyEGj1hmOFoLCyzC0Gi1QsJsti1_gowi3qkeJ9cr1qwTq2J2IZr/exec",
				function(response) {
					var Type = response.type;
					var Province = response.provinces;
					var C_manager = response.c_manager;
					//var Client = response.limerick_clients;
					//var Care_giver = response.limerick_care_givers;
					var Medi_set_type = response.Medi_set_type;
					var Quality_of_Life = response.Quality_of_Life;
					var Expectations = response.Expectations;
					...
					for (var i = 0; i < Expectations.length; i++) {
						if (Expectations[i] != "") {
							$('[name="Expectations"]').append(
								'<option value="' + Expectations[i] + '">' + Expectations[i] + "</option>"
							);
						}
					}
					for (var i = 0; i < Who.length; i++) {
						if (Who[i] != "") {
							$('[name="Who"]').append(
								'<option value="' + Who[i] + '">' + Who[i] + "</option>"
							);
						}
					}
					for (var i = 0; i < Quality_of_Life.length; i++) {
						if (Quality_of_Life[i] != "") {
							$('[name="Quality_of_Life"]').append(
								'<option value="' + Quality_of_Life[i] + '">' + Quality_of_Life[i] + "</option>"
							);
						}
					}
					for (var i = 0; i < Medi_set_type.length; i++) {
						if (Medi_set_type[i] !== "") {
							$(".add_other").append(
								'<option value="' + Medi_set_type[i] + '">' + Medi_set_type[i] + '</option>'
							);
						}
					}

					for (var i = 0; i < C_manager.length; i++) {
						if (C_manager[i] !== "") {
							$('[name="C_manager"]').append(
								'<option value="' +
								C_manager[i] +
								'">' +
								C_manager[i] +
		...
		$('[name="care_manager_id"]').on('change', function() {
			let url = '/find/care-manager/' + $(this).val();
			...
			if (validate==true) {
                ...
                $.ajax({
                    ...
                    data: $(form).serialize() +
                        ...
                        '&attitude=' + attitude.slice("\n", -1) +
                        '&ability=' + ability.slice("\n", -1) +
                        '&reliability=' + reliability.slice("\n", -1) +
                        '&Average_giver=' + Average_giver.slice("\n", -1) +
                        '&CG_comment=' + CG_comment.slice("\n", -1),
	...
	<script>
		$(document).ready(function() {
			...
			$('#county').on('change', function() {
				...
				let id = $(this).val();
				let url = '/find-locality/' + id;
				let url_province = '/find-province/' + id;
				$.get(url, function(data) {
					$('#locality').html(data);
				});
			...
			$(document).on("click", ".search-care-giver", function() {
				...
				if (care_id != undefined) {
					let url = '/find-care-giver/' + care_id;
					$.get(url, function(data) {
						console.log(data[0]);
